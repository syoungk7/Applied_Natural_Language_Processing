...
class Word2Vec(object):
    ...
    def tokenize(self, data):
        ...
        Args: data: (N,) list of sentences in the dataset.
        Return: tokens: (N, D_i) list of tokenized sentences. The i-th sentence has D_i words after the split.
        """
        tokens = []
        for sen in data:
            tokens.append(sen.split())
        return tokens
    ...
    def create_vocabulary(self, tokenized_data):
        ...
        Args: tokenized_data: (N, D) list of split tokens in each sentence.
        Return: None (The update is done for self.word2idx, self.idx2word and self.vocabulary_size)
        """
        tokens = set()
        for sen in tokenized_data:
            for word in sen:
        ...
        self.word2idx = {val: idx for idx, val in enumerate(tokens)}
        self.idx2word = {idx: val for idx, val in enumerate(tokens)}
    ...
    def skipgram_embeddings(self, tokenized_data, window_size=2):
        ...
        Args: tokenized_data: (N, D_i) list of split tokens in each sentence.
              window_size: length of the window for creating context. Default is 2.
        Returns: source_tokens: List of elements where each element is the middle word in the window.
                 target_tokens: List of elements representing IDs of the context words.
        """
        source_tokens, target_tokens = [], []
        for i in tokenized_data:
            for j in range(len(i)):
                tmp = [i[j-k-1] for k in range(window_size-1, -1, -1) if j-k-1 >= 0 ] + \
                    [i[j+k+1] for k in range(window_size) if j+k+1 < len(i)]
                for word in tmp:
                    source_tokens.append([self.word2idx[i[j]]])
    ...
    def cbow_embeddings(self, tokenized_data, window_size=2):
        ...
        for i in tokenized_data:
            for j in range(len(i)):
                target_tokens.append([self.word2idx[i[j]]])
                tmp = [i[j-k-1] for k in range(window_size-1, -1, -1) if j-k-1 >= 0 ] + \
                    [i[j+k+1] for k in range(window_size) if j+k+1 < len(i)]
                source_tokens.append([self.word2idx[word] for word in tmp])
        return source_tokens, target_tokens
...
class SkipGram_Model(nn.Module):
    def __init__(self, vocab_size: int):
        ...
        Reference: embedding - https://pytorch.org/docs/stable/generated/torch.nn.Embedding.html
    ...
    def forward(self, inputs):
        ...
        Args: inputs: (B, 1) Tensor of IDs for each sentence. Where B = batch size
        Returns: output: (B, V) Tensor of logits where V = vocab_size.
        Hint: No need to have a softmax layer here.
        """
...
class CBOW_Model(nn.Module):
    ...
    def forward(self, inputs):
        ...
        Args: inputs: (D_i) Tensor of IDs for each sentence.
        Returns: output: (1, V) Tensor of logits
        Hints: The keepdim parameter may be helpful if using torch.mean
               No need to have a softmax layer here.
        """
        return self.linear(torch.mean(self.embeddings(inputs), -2, True))