...
class Metrics(object):
	...
	def accuracy(self, y, y_hat):
		...
		Args: y_hat: (N,) numpy vector of predicted labels
			  y: list of N labels
	...
	def recall(self, y, y_hat, average='macro'):
		...
		Return: recall: scalar or list of scalars with length of # of unique labels
		'''
		return recall_score(list(y), y_hat, average=average)
	...
	def precision(self, y, y_hat, average='macro'):
		...
		Args: y_hat: (N,) numpy vector of predicted labels
			  y: list of N labels
			  average: None or string 'macro'
		Return: precision: scalar or list of scalars with length of # of unique labels
		'''
		return precision_score(list(y), y_hat, average=average)
	...
	def f1_score(self, y, y_hat, average='macro'):
		...
		'''
		return f1_score(list(y), y_hat, average=average)
	...
	def roc_auc_score(self, y, y_hat, average='macro'):
		...
		'''
		return roc_auc_score(list(y), y_hat, average=average)
	...
	def confusion_matrix(self, y, y_hat):
		...
		Args: y_hat: (N,) numpy vector of predicted labels
			  y: list of N labels
		Return: confusion_matrix: numpy array of the predictions vs ground truth counts.
		'''
		return confusion_matrix(list(y), y_hat)